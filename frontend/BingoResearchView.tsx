...
export function BingoResearchView() {
  ...
  return <div className="h-full flex flex-col min-h-0 bg-background text-foreground">
    ...
    <div className="flex-1 min-h-0"><CustomScrollbar orientation="vertical"><div className="mx-auto max-w-4xl space-y-3 p-3 pb-20 sm:p-4 sm:pb-4">
      <div className="rounded-2xl border border-border bg-card p-3 shadow-sm sm:p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[11px] text-muted-foreground">研究模式 · 回測與娛樂參考</div><div className="mt-1 text-base font-semibold leading-snug sm:text-lg">各玩法只顯示最佳模型</div><div className="mt-1 hidden text-xs text-muted-foreground sm:block">新期別會自動 walk-forward 演化參數</div></div><div className="shrink-0 text-right"><div className="text-[11px] text-muted-foreground">台北時間</div><div className="tabular-nums text-sm font-semibold sm:text-base">{taipeiTime}</div><div className="mt-0.5 text-[11px] text-muted-foreground">下期倒數 {formatCountdown(nextDraw.getTime() - now.getTime())}</div></div></div><div className="mt-3 hidden flex-wrap gap-1 border-t border-border pt-2 sm:flex">{pageButton('overview', '總覽')}{pageButton('plays', '最佳玩法')}{pageButton('process', '計算過程')}{pageButton('history', '歷史紀錄')}</div></div>
      {error && <div className="rounded-lg border border-destructive bg-destructive/10 p-3 text-sm text-destructive">{error}</div>}
      {page === 'overview' && <>
        <section className="rounded-2xl border border-border bg-card p-3 shadow-sm sm:p-4"><div className="flex items-center justify-between gap-2"><h2 className="font-semibold">最新開獎</h2><span className="text-[11px] text-muted-foreground">已保存 {sorted.length} 期{lastSync ? ` · ${new Date(lastSync).toLocaleTimeString('zh-TW')}` : ''}</span></div>{latest ? <><div className="mt-2 text-sm">第 <b className="tabular-nums">{latest.period}</b> 期 · 開獎 {latest.drawAt || '時間未知'}</div><div className="truncate text-[11px] text-muted-foreground">資料來源：{latest.sourceLabel || '來源未知'}</div><div className="mt-3 grid grid-cols-5 gap-1.5">{latest.numbers.split(',').map((number) => <span key={number} className="rounded-lg bg-muted px-1 py-1.5 text-center text-xs tabular-nums">{number}</span>)}</div><div className="mt-3 grid grid-cols-3 gap-2 text-xs sm:text-sm"><div>超級獎號 <b>{latest.superNumber || '—'}</b></div><div>大小 <b>{latest.size || '—'}</b></div><div>單雙 <b>{latest.oddEven || '—'}</b></div></div></> : <p className="mt-2 text-sm text-muted-foreground">等待首次同步。</p>}</section>
        <section className="rounded-2xl border border-border bg-card p-3 shadow-sm sm:p-4"><h2 className="font-semibold">最佳模型摘要</h2><div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">{bestPlays.slice(0, 3).map((play) => <div key={play.key} className="rounded-xl bg-muted/50 p-3"><div className="text-xs text-muted-foreground">{play.label}</div><div className="mt-1 font-semibold">{play.best.model}</div><div className="text-sm">勝率 <Rate value={play.best.rate} /> · {play.best.prediction}</div></div>)}</div><Button className="mt-3 w-full sm:w-auto" variant="outline" size="sm" onClick={() => setPage('plays')}>查看全部玩法</Button></section>
      </>}
      {page === 'plays' && <section className="rounded-2xl border border-border bg-card p-3 shadow-sm sm:p-4"><div className="flex items-end justify-between gap-2"><div><h2 className="font-semibold">各玩法最佳模型</h2><p className="mt-1 text-xs text-muted-foreground">依保存歷史計算，平手時採模型排列優先順序。</p></div><span className="shrink-0 text-[11px] text-muted-foreground">樣本 {sorted.length} 期</span></div><div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">{bestPlays.map((play) => <div key={play.key} className="rounded-xl border border-border/70 p-3"><div className="flex items-center justify-between"><span className="font-medium">{play.label}</span><Rate value={play.best.rate} /></div><div className="mt-1 text-sm">最佳：<b>{play.best.model}</b> · 命中 {play.best.hits}/{play.best.samples}</div><div className="mt-2 break-words rounded-lg bg-muted/50 p-2 text-sm">本期預測：<b>{play.best.prediction}</b></div></div>)}</div></section>}
      {page === 'process' && <section className="rounded-2xl border border-border bg-card p-3 shadow-sm sm:p-4"><h2 className="font-semibold">計算過程</h2><div className="mt-3 space-y-2 text-sm"><div className="rounded-xl border-l-4 border-primary bg-muted/50 p-3"><b>1. 傳統起卦層</b><p className="mt-1 text-xs leading-5 text-muted-foreground">梅花易數以年月日時定上下卦與動爻；六爻以固定種子模擬六次三枚硬幣；河圖洛書用洛書九宮定宮位。</p></div><div className="rounded-xl border-l-4 border-primary bg-muted/50 p-3"><b>2. 自動演化層</b><p className="mt-1 text-xs leading-5 text-muted-foreground">測試五組歷史頻率權重，以 walk-forward 驗證選出下一期參數。樣本少於 4 期時維持預設值。</p></div><div className="rounded-xl border-l-4 border-primary bg-muted/50 p-3"><b>3. 星級玩法命中定義</b><p className="mt-1 text-xs leading-5 text-muted-foreground">N 星必須由模型提出的 N 個號碼全部出現在該期 20 個號碼內。</p></div><div className="rounded-xl border-l-4 border-primary bg-muted/50 p-3"><b>4. 本期實際計算紀錄</b>{latestModels.map((model) => <div key={model.name} className="mt-3 rounded-lg border border-border/60 bg-background/60 p-2.5"><div className="font-medium">{model.name}</div><div className="mt-1 text-xs leading-5 text-muted-foreground">{model.rule}</div><div className="mt-2 break-words text-xs leading-5">{model.calculation?.formula || '舊版紀錄沒有保存公式'}{model.calculation?.historySamples != null ? ` · 前期樣本 ${model.calculation.historySamples} 期 · 頻率權重 ${model.calculation.empiricalWeight}` : ''}</div><div className="mt-1 text-xs">演化狀態：{model.calculation?.evolution?.status || '舊版紀錄沒有演化資訊'}{model.calculation?.evolution?.score != null ? ` · 驗證分數 ${(model.calculation.evolution.score * 100).toFixed(1)}%` : ''}</div><div className="mt-1 break-words text-xs leading-5">候選號碼：{model.research.numberPicks.join('、')} · 區間：{model.research.zones.join('、')} · 總和：{model.research.sumBand}</div></div>)}</div><div className="rounded-xl border-l-4 border-primary bg-muted/50 p-3"><b>5. 勝率公式</b><p className="mt-1 font-mono text-xs leading-5">勝率 = 命中期數 ÷ 有效預測期數 × 100%；回測結果不代表下一期保證。</p></div></div></section>}
      {page === 'history' && <section className="rounded-2xl border border-border bg-card p-3 shadow-sm sm:p-4"><h2 className="font-semibold">歷史開獎與預測</h2><p className="mt-1 text-xs text-muted-foreground">開獎與預測快照已持久化保存；刪除後才會從歷史移除。</p><div className="mt-3 space-y-2">{sorted.slice(0, 50).map((draw) => <div key={draw.id} className="rounded-xl border border-border/70 p-3"><div className="flex items-start justify-between gap-2 text-sm"><div><b>第 {draw.period} 期</b><div className="mt-1 text-[11px] text-muted-foreground">開獎 {draw.drawAt || '時間未知'} · {draw.syncStatus}</div></div><Button variant="ghost" size="sm" onClick={() => void useItemStore.getState().removeItems([draw.id], { needSync: true })}>刪除</Button></div><div className="mt-2 break-words text-xs leading-5">號碼：{draw.numbers} · 大小／單雙：{draw.size || '—'}／{draw.oddEven || '—'} · 預測模型：{parseModels(draw).map((model) => model.name).join('、') || '—'}</div></div>)}{sorted.length === 0 && <p className="text-sm text-muted-foreground">尚無歷史紀錄。</p>}</div></section>}
    ...
    <div className="fixed inset-x-0 bottom-0 z-20 flex gap-1 border-t border-border bg-background/95 p-1 pb-[max(0.25rem,env(safe-area-inset-bottom))] shadow-lg backdrop-blur sm:hidden">{mobilePageButton('overview', '總覽')}{mobilePageButton('plays', '玩法')}{mobilePageButton('process', '過程')}{mobilePageButton('history', '歷史')}</div>
  </div>;
}
