...
export function BingoResearchView() {
  ...
  return <div className="h-full flex flex-col min-h-0 bg-slate-950 text-slate-100">
    ...
    <div className="flex-1 min-h-0"><CustomScrollbar orientation="vertical"><div className="mx-auto max-w-4xl space-y-3 p-3 pb-20 sm:p-4 sm:pb-4">
      ...
      {page === 'overview' && <>
        <section className="rounded-2xl border border-cyan-400/30 bg-slate-900 p-3 shadow-lg shadow-cyan-950/20 sm:p-4"><div className="flex items-center justify-between gap-2"><h2 className="font-semibold text-cyan-300">最新開獎</h2><span className="text-[11px] text-slate-400">已保存 {sorted.length} 期{lastSync ? ` · ${new Date(lastSync).toLocaleTimeString('zh-TW')}` : ''}</span></div>{latest ? <><div className="mt-2 text-sm">第 <b className="tabular-nums text-cyan-200">{latest.period}</b> 期 · 開獎 {latest.drawAt || '時間未知'}</div><div className="truncate text-[11px] text-slate-400">資料來源：{latest.sourceLabel || '來源未知'}</div><div className="mt-3 grid grid-cols-5 gap-1.5">{latest.numbers.map((number) => <span key={number} className="rounded-lg border border-slate-700 bg-slate-800 px-1 py-1.5 text-center text-xs tabular-nums">{number}</span>)}</div><div className="mt-3 grid grid-cols-3 gap-2 text-xs text-slate-300 sm:text-sm"><div>超級獎號 <b className="text-amber-300">{latest.superNumber || '—'}</b></div><div>大小 <b className="text-cyan-300">{latest.size || '—'}</b></div><div>單雙 <b className="text-cyan-300">{latest.oddEven || '—'}</b></div></div></> : <p className="mt-2 text-sm text-slate-400">等待首次同步。</p>}</section>
        ...
        <section className="rounded-2xl border border-cyan-400/30 bg-slate-900 p-3 shadow-lg shadow-cyan-950/20 sm:p-4"><div className="flex items-end justify-between gap-2"><div><h2 className="font-semibold text-cyan-300">最佳玩法推薦</h2><p className="mt-1 text-xs text-slate-400">每個玩法直接套用該玩法歷史勝率最高模型。</p></div><span className="shrink-0 text-[11px] text-slate-400">樣本 {sorted.length} 期</span></div><div className="mt-3 overflow-hidden rounded-xl border border-slate-700"><div className="grid grid-cols-[1.1fr_1fr_0.8fr] gap-2 border-b border-slate-700 px-3 py-2 text-[11px] text-slate-400"><span>玩法／採用模型</span><span>歷史命中率</span><span className="text-right">本期預測</span></div>{bestPlays.map((play) => <div key={play.key} className="grid grid-cols-[1.1fr_1fr_0.8fr] items-center gap-2 border-b border-slate-800 px-3 py-2.5 last:border-b-0"><div className="min-w-0"><div className="font-medium text-white">{play.label}</div><div className="truncate text-[11px] text-slate-400">採用 {play.best.model} · {play.best.samples} 期</div></div><div><div className="text-sm font-semibold text-cyan-200"><Rate value={play.best.rate} /></div><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-slate-700"><div className="h-full rounded-full bg-cyan-300" style={{ width: `${Math.round((play.best.rate ?? 0) * 100)}%` }} /></div></div><div className="break-words text-right text-lg font-semibold text-cyan-300">{play.best.prediction}</div></div>)}</div></section>
      </>}
      ...
      {sourceHealth.length > 0 && <div className="text-xs text-slate-500">資料來源：{sourceHealth.filter((source) => source.ok).map((source) => source.name).join('、')} · {backup?.enabled ? `算法／權重已備份至 ${backup.repo || 'GitHub'}` : '算法／權重 GitHub 備份尚未啟用'}</div>}
