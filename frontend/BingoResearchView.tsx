...
export function BingoResearchView() {
  ...
  return <div className="h-full min-w-0 max-w-full flex flex-col min-h-0 overflow-x-hidden bg-slate-950 text-slate-100">
    ...
    <div className="flex-1 min-h-0 min-w-0 overflow-x-hidden"><CustomScrollbar orientation="vertical"><div className="mx-auto min-w-0 max-w-4xl space-y-3 overflow-x-hidden p-3 pb-20 sm:p-4 sm:pb-4">
      ...
      {page === 'overview' && <>
        <section className="rounded-2xl border border-orange-400/30 bg-slate-900 p-3 shadow-lg shadow-orange-950/20 sm:p-4"><div className="flex items-center justify-between gap-2"><h2 className="font-semibold text-orange-300">最新開獎號碼</h2><span className="shrink-0 text-lg font-semibold tabular-nums text-orange-200">{latest ? `第 ${latest.period} 期` : '等待同步'}</span></div>{latest ? <><div className="mt-3 grid grid-cols-10 gap-1.5">{latest.numbers.map((number, index) => <span key={`${number}-${index}`} className="flex aspect-square items-center justify-center rounded-full border border-orange-200/80 bg-gradient-to-br from-orange-300 via-orange-500 to-orange-700 text-[11px] font-bold tabular-nums text-white shadow-[0_2px_6px_rgba(249,115,22,0.35)] sm:text-xs">{number}</span>)}</div><div className="mt-3 flex items-center justify-between rounded-xl border border-red-400/40 bg-red-950/20 px-3 py-2"><span className="text-xs font-medium text-red-200 sm:text-sm">超級獎號</span><span className="flex h-9 w-9 items-center justify-center rounded-full border border-red-200/90 bg-gradient-to-br from-red-400 via-red-600 to-red-800 text-sm font-bold tabular-nums text-white shadow-[0_2px_8px_rgba(239,68,68,0.4)]">{latest.superNumber || '—'}</span></div></> : <p className="mt-2 text-sm text-slate-400">等待首次同步。</p>}</section>
        <section className="min-w-0 max-w-full rounded-2xl border border-amber-300/40 bg-slate-900 p-3 shadow-lg shadow-amber-950/20 sm:p-4"><div className="flex items-end justify-between gap-2"><h2 className="font-semibold text-amber-300">本期預測玩法、勝率與號碼</h2><span className="shrink-0 text-[10px] text-slate-500">歷史統計</span></div><div className="mt-2 min-w-0 max-w-full divide-y divide-slate-800 overflow-hidden rounded-xl border border-slate-700 bg-slate-950/50"><div className="grid grid-cols-[4.5rem_3.5rem_minmax(0,1fr)] gap-2 border-b border-slate-700 px-2.5 py-1.5 text-[10px] text-slate-500 sm:grid-cols-[6rem_4rem_minmax(0,1fr)]"><span>玩法</span><span>勝率</span><span className="text-right">預測號碼</span></div>{bestPlays.map((play) => <div key={play.key} className="grid min-w-0 max-w-full grid-cols-[4.5rem_3.5rem_minmax(0,1fr)] items-center gap-2 px-2.5 py-2 sm:grid-cols-[6rem_4rem_minmax(0,1fr)]"><span className="shrink-0 whitespace-nowrap text-xs text-slate-300 sm:text-sm">{play.label}</span><span className="text-xs font-semibold tabular-nums text-amber-200 sm:text-sm"><Rate value={play.best.rate} /></span><div className="block min-w-0 max-w-full overflow-x-auto"><span className="block w-max min-w-full whitespace-nowrap text-right text-sm font-semibold tabular-nums text-cyan-300 sm:text-base">{play.best.prediction}</span></div></div>)}</div></section>
      </>}
