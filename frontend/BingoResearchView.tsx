...
export function BingoResearchView() {
  ...
  return <div className="h-full flex flex-col min-h-0 bg-slate-950 text-slate-100">
    ...
    <div className="fixed inset-x-0 bottom-0 z-20 flex gap-1 border-t border-cyan-400/30 bg-slate-950/95 p-1 pb-[max(0.25rem,env(safe-area-inset-bottom))] shadow-lg backdrop-blur sm:hidden">{mobilePageButton('overview', '總覽')}{mobilePageButton('plays', '玩法')}{mobilePageButton('process', '過程')}{mobilePageButton('history', '歷史')}</div>
  </div>;
}
