...
type Model = { name: string; rule: string; calculation?: { formula?: string; historySamples?: number; empiricalWeight?: number }; official: { size: string; oddEven: string; superNumber: string; basic: Record<string, string[]> }; research: { numberPicks: string[]; sumBand: string; oddEvenCount: string; highLowCount: string; zones: string[] } };
...
export function BingoResearchView() {
  ...
  return <div className="h-full flex flex-col min-h-0 bg-background text-foreground">
    ...
    <div className="flex-1 min-h-0"><CustomScrollbar orientation="vertical"><div className="p-3 space-y-3 max-w-4xl mx-auto">
      ...
      {page === 'process' && <section className="rounded-xl border border-border bg-card p-4"><h2 className="font-semibold">計算過程</h2><div className="mt-3 space-y-3 text-sm"><div className="rounded-lg bg-muted/50 p-3"><b>1. 傳統起卦層</b><p className="mt-1 text-muted-foreground">梅花易數以年月日時定上下卦與動爻；六爻以固定種子模擬六次三枚硬幣、由下往上記錄陰陽與動爻；河圖洛書用洛書九宮（4 9 2／3 5 7／8 1 6）定宮位。</p></div><div className="rounded-lg bg-muted/50 p-3"><b>2. 統計校準層</b><p className="mt-1 text-muted-foreground">起卦特徵只提供可重現的排序訊號，再與先前期數的時間衰減頻率混合；歷史樣本不足時會明確顯示樣本數，不宣稱玄學已證明能預測隨機開獎。</p></div><div className="rounded-lg bg-muted/50 p-3"><b>3. 星級玩法命中定義</b><p className="mt-1 text-muted-foreground">N 星視為命中：模型提出的 N 個號碼全部出現在該期 20 個開獎號碼內；不是只中其中一部分。</p></div><div className="rounded-lg bg-muted/50 p-3"><b>4. 本期實際計算紀錄</b>{latestModels.map((model) => <div key={model.name} className="mt-2 border-t border-border/60 pt-2"><div className="font-medium">{model.name}</div><div className="mt-1 text-xs text-muted-foreground">{model.rule}</div><div className="mt-1 text-xs">{model.calculation?.formula || '舊版紀錄沒有保存公式'}{model.calculation?.historySamples != null ? ` · 使用前期樣本 ${model.calculation.historySamples} 期 · 頻率權重 ${model.calculation.empiricalWeight}` : ''}</div><div className="mt-1 text-xs">候選號碼：{model.research.numberPicks.join('、')} · 區間：{model.research.zones.join('、')} · 總和：{model.research.sumBand}</div></div>)}</div><div className="rounded-lg bg-muted/50 p-3"><b>5. 勝率公式</b><p className="mt-1 font-mono text-xs">勝率 = 命中期數 ÷ 有效預測期數 × 100%；回測結果不代表下一期保證。</p></div></div></section>}
