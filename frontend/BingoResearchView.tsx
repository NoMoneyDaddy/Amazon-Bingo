...
export function BingoResearchView() {
  ...
  return <div className="h-full flex flex-col min-h-0 bg-slate-950 text-slate-100">
    ...
    <div className="flex-1 min-h-0"><CustomScrollbar orientation="vertical"><div className="mx-auto max-w-4xl space-y-3 p-3 pb-20 sm:p-4 sm:pb-4">
      ...
      {page === 'overview' && <>
        <section className="rounded-2xl border border-cyan-400/30 bg-slate-900 p-3 shadow-lg shadow-cyan-950/20 sm:p-4"><div className="flex items-center justify-between gap-2"><h2 className="font-semibold text-cyan-300">最新開獎號碼</h2><span className="shrink-0 text-lg font-semibold tabular-nums text-cyan-200">{latest ? `第 ${latest.period} 期` : '等待同步'}</span></div>{latest ? <div className="mt-3 grid grid-cols-10 gap-1">{latest.numbers.map((number) => <span key={number} className="rounded-md border border-slate-700 bg-slate-800 px-0.5 py-1.5 text-center text-[11px] tabular-nums sm:text-xs">{number}</span>)}</div> : <p className="mt-2 text-sm text-slate-400">等待首次同步。</p>}</section>
        <section className="rounded-2xl border border-amber-300/40 bg-slate-900 p-3 shadow-lg shadow-amber-950/20 sm:p-4"><h2 className="font-semibold text-amber-300">本期預測玩法與號碼</h2><div className="mt-2 divide-y divide-slate-800 overflow-hidden rounded-xl border border-slate-700 bg-slate-950/50">{bestPlays.map((play) => <div key={play.key} className="grid min-w-0 grid-cols-[4.5rem_minmax(0,1fr)] items-center gap-2 px-2.5 py-2 sm:grid-cols-[6rem_minmax(0,1fr)]"><span className="shrink-0 whitespace-nowrap text-xs text-slate-300 sm:text-sm">{play.label}</span><div className="min-w-0 overflow-x-auto"><span className="whitespace-nowrap text-sm font-semibold tabular-nums text-cyan-300 sm:text-base">{play.best.prediction}</span></div></div>)}</div></section>
      </>}
